...
@else
<meta property="og:image" content="{{asset('public/img/'.$info->logo)}}" />
@endif
...
<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
 <!--<script>
  (adsbygoogle = window.adsbygoogle || []).push({
   ...
   <!-- CSS -->
    <link rel="stylesheet" href="{{asset('public/frontend/css/bootstrap.min_2.css')}}" >
    <link rel="stylesheet" href="{{asset('public/frontend/css/font-awesome.min_2.css')}}">
	<link rel="stylesheet" href="{{asset('public/frontend/css/icofont.css')}}">
    <link rel="stylesheet" href="{{asset('public/frontend/css/owl.carousel_2.css')}}">  
    <link rel="stylesheet" href="{{asset('public/frontend/css/slidr_2.css')}}">     
    <link rel="stylesheet" href="{{asset('public/frontend/css/main_2.css')}}">  
	<link id="preset" rel="stylesheet" href="{{asset('public/frontend/css/presets/preset1.css')}}">	
		...
		<nav class="navbar navbar-default">
			<div class="container">
				...
				<div class="navbar-header">
					...
					<a class="navbar-brand" href="{{URL::to('/')}}"><img class="img-responsive" src='{{asset("public/img/$info->logo")}}' alt="{{$info->company_name}}"></a>
				</div>
				<!-- /navbar-header -->
				...
				<div class="navbar-left">
					<div class="collapse navbar-collapse" id="navbar-collapse">
                        ...
                        ?>
							<li class="<? echo($url==$menu->url)?'active' : '' ?> {{(count($subMenus)>0)?'dropdown' : ''}}"><a href='{{URL::to("$menu->url")}}' class="{{(count($subMenus)>0)?'dropdown-toggle' : ''}}" data-toggle="{{(count($subMenus)>0)?'dropdown' : ''}}">{{$menu->name}} {{(count($subMenus)>0)?'<span class="caret"></span>' : ''}}</a>
							@if(count($subMenus)>0)
								<ul class="dropdown-menu">
								@foreach($subMenus as $subMenu)
									<li class="<? echo($url==$menu->url)?'active' : '' ?>"><a href='{{URL::to("$subMenu->url")}}'>{{$subMenu->name}}</a></li>
								@endforeach
								</ul>
							@endif
							</li>
						@endforeach
						<li>
						<a href="{{URL::to('ad-post')}}" class="btn-custom">আপনার বিজ্ঞাপন দিন
 </a>
						</li>
						</ul>
					</div>
				...
				<div class="nav-right">
					...
					<!-- sign-in -->	
					<ul class="sign-in">
					<a href="{{URL::to('ad-post')}}" class="btn btn-custom"> Ad Post <span class="hidden-mb"> Ad/ Jobs</span></a>				
						@if(Auth::check())
						<li class="dropdown">
